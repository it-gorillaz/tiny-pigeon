...
class SendEmailUseCase {
  ...
  async execute(request) {
    ...
    if (EmailType.HTML_TEMPLATE_FILE === request.emailType) {
      ...
      request.body.content = new TextDecoder().decode(content);
    }

    const subject = transform(request.subject.content, request.subject.params);
    const body = transform(request.body.content, request.body.params);
